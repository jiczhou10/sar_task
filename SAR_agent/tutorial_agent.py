...
from matrx.actions import RemoveObject, GrabObject, DropObject
from matrx.messages.message import Message
from matrx.utils import get_distance
from matrx.agents.agent_utils.state import State
from matrx.agents.agent_utils.state_tracker import StateTracker
from matrx.agents.agent_utils.navigator import Navigator
from action.agent_actions import RemoveAlone, CarryAlone, CarryTogether, RemoveTogether, RemoveAgents, DropAlone
...
class Phase(enum.Enum):
    ...
    INTRO18 = 38,
    CARRY_TOGETHER1 = 39,
    WAIT_FOR_SAFE_ZONE1 = 40,
    WAIT_FOR_REMOVE_ROCK = 41,
    END_GAME = 42
...
class TutorialAgent(AgentBrain):
    def __init__(self, **kwargs):
        ...
        self.simulation_start_at_tick = 9999999999
    ...
    def _send_message(self, message):
        # if previous sendMessages is empty
    ...
    def decide_on_action(self, state: State):
        # action = None
        # kwargs = {"action_duration": 20}

        while True:
            self.safe_tiles = self.get_safe_zone_tiles(state)
            # check whether human asks for help
            # prev_received_message_len = len(self.received_messages)

            if self.received_messages and self.received_messages[-1].content == 'Help' \
                    and self.phase != Phase.GO_TO_HUMAN and self.phase != Phase.HELP_HUMAN \
                    and self.phase != Phase.INTRO \
                    and self.phase != Phase.INTRO2 \
                    and self.phase != Phase.INTRO3 \
                    and self.phase != Phase.INTRO4 \
                    and self.phase != Phase.INTRO5 \
                    and self.phase != Phase.INTRO6 \
                    and self.phase != Phase.INTRO7 \
                    and self.phase != Phase.INTRO8 \
                    and self.phase != Phase.INTRO9 \
                    and self.phase != Phase.INTRO10 \
                    and self.phase != Phase.INTRO11 \
                    and self.phase != Phase.INTRO12 \
                    and self.phase != Phase.INTRO13 \
                    and self.phase != Phase.INTRO14 \
                    and self.phase != Phase.INTRO15 \
                    and self.phase != Phase.INTRO16 \
                    and self._state['World']['nr_ticks']-self.received_messages_ticks[-1] <= 100:
                # record agent's previous phase and location
                self.prevPhase = self.phase
                if self.prevPhase == Phase.GO_TO_LOC:
                    self.agentPrevLoc = self.wayPoint[-1]
                # self.nextPhase = Phase.GO_TO_HUMAN
                else:
                    self.agentPrevLoc = state.get_agents_with_property({"name": "agent"})[0].get('location')
                self._send_message(Message('before helping ' + str(self.prevPhase) + str(self.agentPrevLoc), self.msg_id))
                # go to human's location
                self.phase = Phase.GO_TO_HUMAN

            if self._state['World']['nr_ticks'] - self.simulation_start_at_tick >= 100:
                self._send_message(Message('3_left.svg', self.msg_id))
            if self._state['World']['nr_ticks'] - self.simulation_start_at_tick >= 1300:
                self._send_message(Message('2_left.svg', self.msg_id))
            if self._state['World']['nr_ticks'] - self.simulation_start_at_tick >= 2500:
                self._send_message(Message('1_left.svg', self.msg_id))
            if self._state['World']['nr_ticks'] - self.simulation_start_at_tick >= 3725:
                nr_victims = state.get_agents_with_property({"name": "human"})[0].get('nr_victims')
                self._send_message((Message(f'Congratulations! We successfully rescued {nr_victims} victims! Now the task '
                                            'has ended, please ask the instructor for the next step. ', self.msg_id)))
                self.phase = Phase.END_GAME

            if self.phase == Phase.END_GAME:
                action = RemoveAgents
                return action, {}

            if Phase.INTRO == self.phase:
                mssg = Message(content=f'Hello, my name is SaR bot. In this tutorial, I will introduce some rules of this '
                                       f'search and rescue task. '
                                       f'In this world, you can move with keys '
                                       f'W-A-S-D or the arrow keys. '
                                       f'You can try to walk around with these keys, and once '
                                       f'you are ready for the next tip, you can click the Ready button.'
                               , from_id=self.msg_id)
                self._send_message(mssg)
                if self.received_messages and self.received_messages[-1].content == 'Ready':
                    self.phase = Phase.INTRO2
                    self.received_messages = []
                else:
                    return None, {}

            if self.phase == Phase.INTRO2:
                mssg = Message(
                    content=f'Due to our capability, we can only see each other within 3 blocks. '
                            f'You can try to move around, and get to know this capability. '
                            f'If you have understood this, and you are ready for the next tip, '
                            f'you can click the Ready button.'
                    , from_id=self.msg_id)
                self._send_message(mssg)
                if self.received_messages and self.received_messages[-1].content == 'Ready':
                    self.phase = Phase.INTRO3
                    self.received_messages = []
                else:
                    return None, {}

            if self.phase == Phase.INTRO3:
                mssg = Message(
                    content=f'This world is separated by different rooms, which are surrounded by wall.png . '
                            f'Normal rooms are surrounded by wall.png, and safe zone is surrounded by wall2.png. '
                            f'During this task, we will refer different rooms by their room number. '
                            f'You can go to the sign of A1.svg. '
                            f'If you arrives at A1.svg, '
                            f'you can click the Ready button.'
                    , from_id=self.msg_id)
                self._send_message(mssg)
                self._navigator.reset_full()
                self._navigator.add_waypoint((2, 3))
                self._state_tracker.update(self.state)
                action = self._navigator.get_move_action(self._state_tracker)
                if action != None:
                    return action, {}
                    ...
                    if self.received_messages and self.received_messages[-1].content == 'Ready':
                        self.phase = Phase.INTRO4
                        self.received_messages = []
                    else:
                        return None, {}
                    return None, {}

            if self.phase == Phase.INTRO4:
                mssg = Message(
                    content=f'Now we are standing in front of the entry of room A1.svg. '
                            f'There is a rock1.png blocks the entry of this room. '
                            f'You can only visualize the obstacles within 1 block. '
                            f'There are 3 types of obstacle in this world: rock1.png, tree.png, tall.png. '
                            f'If you are ready for the next tip, '
                            f'you can click the Ready button.'
                    , from_id=self.msg_id)
                self._send_message(mssg)
                if self.received_messages and self.received_messages[-1].content == 'Ready':
                    self.phase = Phase.INTRO5
                    self.received_messages = []
                else:
                    return None, {}

            if self.phase == Phase.INTRO5:
                mssg = Message(
                    content=f'Now we can try to remove this rock. Both of us are able to remove rock1.png alone. '
                            f'We can only remove obstacles within 1 block away. '
                            f'You can try to remove rock1.png alone by pressing R. '
                            f'If the removal is successful, you will see the rock disappear after few seconds. '
                            f'If you have removed rock1.png, you can click the Ready button.'
                    , from_id=self.msg_id)
                self._send_message(mssg)
                if self.received_messages and self.received_messages[-1].content == 'Ready':
                    self.phase = Phase.INTRO6
                    self.received_messages = []
                else:
                    return None, {}

            if self.phase == Phase.INTRO6:
                mssg = Message(
                    content=f'After removing this rock, we can enter room A1.svg. '
                            f'You can try to search for victims in this room. '
                            f'There are three types of victims in this world: '
                            f'healthy_victim.png, injured_victim.png, '
                            f'and critical_victim.png. By successfully rescuing, 1, 3, 6 points '
                            f'will be assigned to each type of victims. '
                            f'If you have found healthy_victim.png, '
                            f'you can click the Ready button.'
                    , from_id=self.msg_id)
                self._send_message(mssg)
                if self.received_messages and self.received_messages[-1].content == 'Ready':
                    self.phase = Phase.INTRO7
                    self.received_messages = []
                else:
                    return None, {}

            if self.phase == Phase.INTRO7:
                human_loc = state.get_agents_with_property({"name": "human"})[0].get('location')
                mssg = Message(
                    content=f'Successfully rescuing a victim means carrying that victim, '
                            f'and then dropped that victim to the safe zone. '
                            f'We are able to carry healthy_victim.png and injured_victim.png alone. '
                            f'But for critical_victim.png, '
                            f'we can only carry them together. '
                            f'If you understands everything, you can click the Ready button. '
                    , from_id=self.msg_id)
                self._send_message(mssg)
                if self.received_messages and self.received_messages[-1].content == 'Ready':
                    self.phase = Phase.INTRO8
                    self.received_messages = []
                else:
                    return None, {}

            if self.phase == Phase.INTRO8:
                human_loc = state.get_agents_with_property({"name": "human"})[0].get('location')
                mssg = Message(
                    content=f'You can press G to grab victims alone. It takes some time '
                            f'for grabbing. Once finished, healthy_victim.png will disappear. Then '
                            f'you can carry this victim to the safe zone. Which is surround by wall2.png. '
                            f'If you understands everything, you can click the Ready button.'
                    , from_id=self.msg_id)
                self._send_message(mssg)
                if self.received_messages and self.received_messages[-1].content == 'Ready':
                    self.phase = Phase.INTRO9
                    self.received_messages = []
                else:
                    return None, {}

            if self.phase == Phase.INTRO9:
                if state.get_agents_with_property({"name": "human"}):
                    human_loc = state.get_agents_with_property({"name": "human"})[0].get('location')
                else:
                    return None, {}
                mssg = Message(
                    content=f'Now we can try to carry healthy_victim.png to safe zone. In this world, victims are considered '
                            f'to be successfully rescued once they are carried to safe zone. '
                            f'Once you arrived safe zone, you can click the Ready button.'
                    , from_id=self.msg_id)
                self._send_message(mssg)
                if self.received_messages and self.received_messages[-1].content == 'Ready':
                    '''
                    check whether human really arrives at safe zone
                    '''
                    if human_loc and human_loc in self.safe_tiles:
                        self.received_messages = []
                        self.phase = Phase.INTRO10
                    else:
                        return None, {}
                else:
                    return None, {}

            if self.phase == Phase.INTRO10:
                human_loc = state.get_agents_with_property({"name": "human"})[0].get('location')
                self._navigator.reset_full()
                self._navigator.add_waypoint((10, 5))
                self._state_tracker.update(self.state)
                action = self._navigator.get_move_action(self._state_tracker)
                if action != None:
                    return action, {'action_duration': 5}
                mssg = Message(
                    content=f'You can drop victim by pressing Q. '
                            f'When dropping, the victim will be dropped at the same location of yours. '
                            f'After you dropped this victim, you can come to entry of A2.svg. We will '
                            f'try to remove an obstacle together. If you have arrived at A2.svg, '
                            f'you can click the Ready button. '
                    , from_id=self.msg_id)
                self._send_message(mssg)
                if self.received_messages and self.received_messages[-1].content == 'Ready':
                    self.phase = Phase.INTRO11
                    self.received_messages = []
                else:
                    return None, {}

            if self.phase == Phase.INTRO11:
                human_loc = state.get_agents_with_property({"name": "human"})[0].get('location')
                mssg = Message(
                    content=f'There is a tree.png blocked the entry of A2.svg. '
                            f'I am not able to remove tree.png. So I suggest you help me remove it together. '
                            f'If you agree to work together, '
                            f'you can click Ok button to indicate your decision. '
                    , from_id=self.msg_id)
                self._send_message(mssg)
                if self.received_messages and self.received_messages[-1].content == 'Ok':
                    self.phase = Phase.INTRO12
                    self.received_messages = []
                else:
                    return None, {}


            if self.phase == Phase.INTRO12:
                mssg = Message(
                    f'Once you are within one block of the obstacles, you can press P '
                    f'to remove an obstacle with me. If you see tree.png has been removed, '
                    f'you can click Ready. '
                    , self.msg_id
                )
                self._send_message(mssg)
                if self.received_messages and self.received_messages[-1].content == 'Ready':
                    self.phase = Phase.INTRO13
                    self.received_messages = []
                else:
                    return None, {}
                return None, {}

            if self.phase == Phase.INTRO13:
                human_loc = state.get_agents_with_property({"name": "human"})[0].get('location')
                self._navigator.reset_full()
                self._navigator.add_waypoint((11, 3))
                self._state_tracker.update(self.state)
                action = self._navigator.get_move_action(self._state_tracker)
                if action != None:
                    return action, {}
                    ...
                    mssg = Message(
                        content=f'There is critical_victim.png in A2.svg. '
                                f'We can only carry critical_victim.png together. '
                                f'I am at the same location of critical_victim.png, '
                                f'If you come to my location, and press key C, '
                                f'you can see our icons changed. After that, you can click Ready. '
                    , from_id=self.msg_id)
                    self._send_message(mssg)
                    if self.received_messages and self.received_messages[-1].content == 'Ready':
                        # self.phase = Phase.CARRY_TOGETHER1
                        self.phase = Phase.INTRO14
                        self.received_messages = []
                    else:
                        return None, {}
                return None, {}

            if self.phase == Phase.INTRO14:
                human_loc = state.get_agents_with_property({"name": "human"})[0].get('location')
                mssg = Message(
                    content=f'Once we are carrying victims together, you will take charge to plan the path to the safe zone. '
                            f'Now you can move to the safe zone, and drop this victim at the safe zone. '
                            f'If you successfully dropped critical_victim.png, you can click the Ready button.'
                    , from_id=self.msg_id)
                self._send_message(mssg)
                if self.received_messages and self.received_messages[-1].content == 'Ready':
                    self.phase = Phase.INTRO15
                    self.received_messages = []
                else:
                    return None, {}

            if self.phase == Phase.INTRO15:
                human_loc = state.get_agents_with_property({"name": "human"})[0].get('location')

                mssg = Message(
                    content=f'There is a tall.png blocks the entry of A3.svg. You cannot remove tall.png by yourself. '
                            f'So if you are within 1 block with any obstacle, you can request my help by clicking '
                            f'Help button. I will go to your location, and if I sensed any type of obstacle or victims '
                            f'to work together, I will wait for your keyboard instruction to work together. Now you can '
                            f'go the entry of A3.svg and try to request Help. '
                    , from_id=self.msg_id)
                self._send_message(mssg)
                if self.received_messages and self.received_messages[-1].content == 'Help':
                    if human_loc == (18, 5):
                        self._navigator.reset_full()
                        self._navigator.add_waypoint((18, 5))
                        self._state_tracker.update(state)
                        action = self._navigator.get_move_action(self._state_tracker)
                        if action != None:
                            return action, {}
                        else:
                            self.phase = Phase.INTRO16
                            self.received_messages = []
                            return None, {}
                    else:
                        return None, {}
            ...
            if self.phase == Phase.INTRO16:
                human_loc = state.get_agents_with_property({"name": "human"})[0].get('location')
                mssg = Message(
                    content=f'Now I have moved to your location. And I sensed there is a tall.png that '
                            f'we can work together. If you want to remove this obstacle together, you can press P. '
                            f'If tall.png has been removed, you can click the Ready button. '
                    , from_id=self.msg_id)
                self._send_message(mssg)
                if self.received_messages and self.received_messages[-1].content == 'Ready':
                    self.phase = Phase.INTRO17
                    self.received_messages = []
                else:
                    return None, {}

            if self.phase == Phase.INTRO17:
                human_loc = state.get_agents_with_property({"name": "human"})[0].get('location')
                mssg = Message(
                    content=f'Now I have introduced all possible actions. We can go to the simulation zone to practice '
                            f'with the mock task before we actually started. If you are Ready for the mocking task, you '
                            f'can click the Ready button and remove the rock1.png between the blue walls. '
                    , from_id=self.msg_id)
                self._send_message(mssg)
                if self.received_messages and self.received_messages[-1].content == 'Ready':
                    obj = None
                    obstacles = state.get_with_property(props={'class_inheritance': 'Obstacle'})
                    # check whether the blocking rock has been removed
                    for obstacle in obstacles:
                        if obstacle['location'] == (14, 8):
                            obj = obstacle
                    if not obj:
                        self.phase = Phase.INTRO18
                        self.received_messages = []
                    else:
                        return None, {}
                else:
            ...
            if self.phase == Phase.INTRO18:
                human_loc = state.get_agents_with_property({"name": "human"})[0].get('location')
                mssg = Message(
                    content=f'We have 3 minutes to work on this simulation task. '
                            f'Once the remaining time is less than 3 minutes, I will send a countdown message '
                            f'every minute in the chat. If you understand '
                            f'everything, you can click the Ready button, and start working on the task. '
                    , from_id=self.msg_id)
                self._send_message(mssg)
                if self.received_messages and self.received_messages[-1].content == 'Ready':
                    self.phase = Phase.GET_CLOSEST_STUCK_ROOM
                    self.received_messages = []
                    self.simulation_start_at_tick = self._state['World']['nr_ticks']
                else:
                    return None, {}
            ...
            if self.phase == Phase.GET_CLOSEST_STUCK_ROOM:
                ...
                if self.next_closest_flag:
                    print('next closest flag is true')
