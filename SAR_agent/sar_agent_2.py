                ...
                if agent_location == self.lastEntry:
                    ...
                    if closest_obstacle and get_distance(closest_obstacle[0]['location'], agent_location) <= 1:
                        ...
                        if type == 'small' and agent_opacity != 0:
                            mssg = Message(f'I found {img} blocking {door_img}. '
                                           f'I am able to remove {img} alone, so '
                                           f'I suggest to remove it by myself. If you agree, I will remove {door_img}, and '
                                           f'search {door_img} for victims. ',
                                           self.msg_id)
                            self._send_message(mssg)
                            # self.received_messages = []
                            ...
                            if distance > 10:
                                mssg = Message(f'I found {img} blocking {door_img}. '
                                               f'I am able to remove {img} myself. Based on '
                                               f'our distance, it would be slower if we work together, so '
                                               f'I suggest to remove it myself. '
                                               f'If you agree, I will start to work alone. ', self.msg_id)
                                self._send_message(mssg)
                                human = state.get_agents_with_property(props={"name":"human"})[0]
                                agent = state.get_agents_with_property(props={"name":"agent"})[0]
                                if self.received_messages and self.received_messages[-1].content == 'Ok':
                            ...
                            # for soft interdependence, agent sends message to request collaborate
                                ...
                                mssg = Message(f'I found {img} blocking {door_img}. '
                                               f'I am able to work alone, but it would be faster if we work together. '
                                               f'After removal, we can enter {door_img} and search for victims. '
                                               f'So I suggest you come here and we remove it together. '
                                               f'If you agree, I will stay here and wait for you. '
                                               , self.msg_id)
                                self._send_message(mssg)
                                if self.received_messages and self.received_messages[-1].content == 'Ok':
                        ...
                        # for hard interdependence, it requires two agents to collaborate to remove large obstacle
                            ...
                            mssg = Message(f'I found {img} blocking {door_img}. '
                                           f'I am not able to remove {img} by myself. '
                                           f'If we remove {img}, we can enter {door_img} to search for victims. '
                                           f'So I suggest you come here and we remove it together. '
                                           f'If you agree, I will stay at {door_img} and wait for you. ',
                                           self.msg_id)
                            self._send_message(mssg)
