...
const time_remaining_div = $("#time_remaining_div")
...
//        console.log("Fps: ", lv_fps);
    ...
    document.getElementById('score').innerHTML = "Score: " + lv_current_score;
...
function get_MATRX_update() {
    ...
    var lv_update_request = $.ajax({
        ...
        success: function(data) {
            ...
            lv_current_score = lv_state['human']['score']
...
async function fill_time_remaining() {
    let seconds_past = parseFloat(lv_state['World']['tick_duration']) * parseInt(lv_state['World']['nr_ticks']);
    time_remaining_div.empty()

    // if(phase_time === 0) {
    //     let new_time = await get_time_limit()
    //     console.log(new_time)
    //     if (new_time != null && new_time !== phase_time) {
    //         phase_time = new_time
    //     }
    // }

    if (!matrx_has_been_paused_for_SA && lv_state['search_robot'] != null &&
        lv_state['search_robot']['custom_properties'] != null &&
        lv_state['search_robot']['custom_properties']['SA'] != null
        && lv_state['search_robot']['custom_properties']['tutorial'] != null
        && lv_state['search_robot']['custom_properties']['tutorial'] === 0
        && lv_state['search_robot']['custom_properties']['SA'] === 1) {
        $("#pause_button").click();
        matrx_has_been_paused_for_SA = true;
        $('#SA_pause_modal').modal({
            backdrop: 'static',
            keyboard: false
        })
    }

    if (!matrx_has_been_stopped && lv_state['search_robot'] != null &&
        lv_state['search_robot']['custom_properties'] != null &&
        lv_state['search_robot']['custom_properties']['stop'] != null
        && lv_state['search_robot']['custom_properties']['stop'] === 1) {
        $("#pause_button").click();
        matrx_has_been_stopped = true;
        $('#SAR_done_modal').modal({
            backdrop: 'static',
            keyboard: false
        })
        let rescued = lv_state['search_robot']['custom_properties']['nr_rescued']
        let max_rescue = lv_state['search_robot']['custom_properties']['max_survivors']
        $('#SAR_done_modal .modal-body').html("Please tell the instructor of the experiment that the experiment is done! Thank you so much!\n" +
            "                    You have rescued " + rescued.length + " out of " + parseInt(max_rescue) + " survivors!")
        $.confetti.start();
        $('#SAR_done_modal').on('hidden.bs.modal', function () {
            $("#stop_button").click();
        });
    }
    ...
    // if (seconds_past > 2) {
    //     console.log("!")
    //     set_rescue_plan(JSON.stringify([{'container': {'node': {'h5s': {'data': {'opts': {'items': null, 'connectWith': null, 'disableIEFix': null, 'acceptFrom': null, 'copy': false, 'placeholder': null, 'placeholderClass': 'mb1 bg-navy border border-yellow', 'draggingClass': 'sortable-dragging', 'hoverClass': false, 'dropTargetContainerClass': false, 'debounce': 0, 'throttleTime': 100, 'maxItems': 0, 'customDragImage': null, 'orientation': 'vertical', 'forcePlaceholderSize': true}, '_disabled': 'false', 'items': null}}, 'isSortable': true}, 'itemCount': 2}, 'items': [{'parent': {'h5s': {'data': {'opts': {'items': null, 'connectWith': null, 'disableIEFix': null, 'acceptFrom': null, 'copy': false, 'placeholder': null, 'placeholderClass': 'mb1 bg-navy border border-yellow', 'draggingClass': 'sortable-dragging', 'hoverClass': false, 'dropTargetContainerClass': false, 'debounce': 0, 'throttleTime': 100, 'maxItems': 0, 'customDragImage': null, 'orientation': 'vertical', 'forcePlaceholderSize': true}, '_disabled': 'false', 'items': null}}, 'isSortable': true}, 'node': {}, 'html': '<li data-index="index" draggable="true" role="option" aria-grabbed="false">Dangerous Area at (13,4)</li>', 'index': 0}, {'parent': {'h5s': {'data': {'opts': {'items': null, 'connectWith': null, 'disableIEFix': null, 'acceptFrom': null, 'copy': false, 'placeholder': null, 'placeholderClass': 'mb1 bg-navy border border-yellow', 'draggingClass': 'sortable-dragging', 'hoverClass': false, 'dropTargetContainerClass': false, 'debounce': 0, 'throttleTime': 100, 'maxItems': 0, 'customDragImage': null, 'orientation': 'vertical', 'forcePlaceholderSize': true}, '_disabled': 'false', 'items': null}}, 'isSortable': true}, 'node': {'h5s': {'data': {'dropped': 'true'}}}, 'html': '<li data-index="index" draggable="true" role="option" aria-grabbed="false" class="" style="">Survivor at (2,9) [Critically Injured]</li>', 'index': 1}]}]))
    // }

    if (!rescue_plan_is_made && lv_state['search_robot'] != null &&
        lv_state['search_robot']['custom_properties'] != null &&
        lv_state['search_robot']['custom_properties']['done'] != null
        && lv_state['search_robot']['custom_properties']['done'] === 1) {


        // If time to make rescue plan
        $('#rescue_plan_modal').modal({
            backdrop: 'static',
            keyboard: false
        })

        // Load todos
        let todos = lv_state['search_robot']['custom_properties']['todos']

        let list = []

        let rescue_plan = $("#rescue_plan_list")

        set_init_rescue_plan(todos, rescue_plan)

        let number_todos = rescue_plan.children().length;

        if (number_todos > 0) {
            sortable('.rescue_plan_list', {
                forcePlaceholderSize: true,
                placeholderClass: 'mb1 bg-navy border border-yellow',
            });
        } else {
            $('#rescue_plan_modal').modal('hide')
        }

        has_emptied_card_rescue_phase = true
        rescue_plan_is_made = true

        $('#rescue_plan_modal').on('hidden.bs.modal', function () {
            let order = sortable('.sortable', 'serialize');
            if (number_todos === 0) {
                // TODO exit SAR
            }
            new_beginning_time = lv_state['search_robot']['custom_properties']['new_beginning_time']

            if (new_beginning_time == null || new_beginning_time === 0) {

                // remove_buttons_give_advice_card()
                // remove_buttons_follow_advice_card()
                // set_rescue_plan(JSON.stringify([{'container': {'node': {'h5s': {'data': {'opts': {'items': null, 'connectWith': null, 'disableIEFix': null, 'acceptFrom': null, 'copy': false, 'placeholder': null, 'placeholderClass': 'mb1 bg-navy border border-yellow', 'draggingClass': 'sortable-dragging', 'hoverClass': false, 'dropTargetContainerClass': false, 'debounce': 0, 'throttleTime': 100, 'maxItems': 0, 'customDragImage': null, 'orientation': 'vertical', 'forcePlaceholderSize': true}, '_disabled': 'false', 'items': null}}, 'isSortable': true}, 'itemCount': 2}, 'items': [{'parent': {'h5s': {'data': {'opts': {'items': null, 'connectWith': null, 'disableIEFix': null, 'acceptFrom': null, 'copy': false, 'placeholder': null, 'placeholderClass': 'mb1 bg-navy border border-yellow', 'draggingClass': 'sortable-dragging', 'hoverClass': false, 'dropTargetContainerClass': false, 'debounce': 0, 'throttleTime': 100, 'maxItems': 0, 'customDragImage': null, 'orientation': 'vertical', 'forcePlaceholderSize': true}, '_disabled': 'false', 'items': null}}, 'isSortable': true}, 'node': {}, 'html': '<li data-index="index" draggable="true" role="option" aria-grabbed="false">Dangerous Area at (13,4)</li>', 'index': 0}, {'parent': {'h5s': {'data': {'opts': {'items': null, 'connectWith': null, 'disableIEFix': null, 'acceptFrom': null, 'copy': false, 'placeholder': null, 'placeholderClass': 'mb1 bg-navy border border-yellow', 'draggingClass': 'sortable-dragging', 'hoverClass': false, 'dropTargetContainerClass': false, 'debounce': 0, 'throttleTime': 100, 'maxItems': 0, 'customDragImage': null, 'orientation': 'vertical', 'forcePlaceholderSize': true}, '_disabled': 'false', 'items': null}}, 'isSortable': true}, 'node': {'h5s': {'data': {'dropped': 'true'}}}, 'html': '<li data-index="index" draggable="true" role="option" aria-grabbed="false" class="" style="">Survivor at (2,9) [Critically Injured]</li>', 'index': 1}]}]))
                rescue_phase = true
                new_beginning_time = seconds_past
                set_rescue_plan(order)
                set_new_time(new_beginning_time)
            } else {
                rescue_phase = true
                console.log("new time: " + new_beginning_time)
            }
        });

    }
    if (phase_time === 0) {
        phase_time = lv_state['search_robot']['custom_properties']['time_limit']
        console.log("phase time: " + phase_time)
    }
    ...
    if (rescue_phase) {
        ...
        time_remaining = Math.round(phase_time - seconds_past + new_beginning_time);
    } else {
        time_remaining = Math.round(phase_time - seconds_past);
    }
    if (time_remaining < 0) {
        time_remaining = 0
    }
    time_remaining_div.append("Time remaining: " + time_remaining)
}
